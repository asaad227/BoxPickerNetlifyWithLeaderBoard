...
async function fetchPostUsers() {
    ...
    let response = await fetch(`/.netlify/functions/post_users`, {
      method: "POST",
      headers: {
  ...
  async function getUsers() {
    ...
    if(data.length <=  1){
        ...
        nam.innerHTML = data[0].name;
        total.innerHTML = data[0].score
    }
...
if(data.length > 1){
        ...
        console.log(sorted)
        const nameMap = sorted.map(e=> e.name);
        const scoreMap = sorted.map(e=> e.score);
